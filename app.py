...
# Safe execution check avoiding passing None objects directly to np.isnan
...
# Cache lookup parameter key rounded to 2 decimal places
...
def load_and_compute_gex_engine(ticker, r_rate, target_price, min_d, max_d):
    try:
        ...
        # Enforce historical 3 trading row minimum for long holiday weekends
        try:
            vix = yf.Ticker("^VIX")
...
# Strict control flow intercept if filters yield an empty dataset
if data_matrix is None:
    st.error("❌ No options data matched your DTE filter range. Try widening Min/Max DTE filters in the sidebar.")
    st.stop()
...
# UI Charm notification layout
total_charm = data_matrix['Charm'].sum()
charm_display = format_scaled_exposure(total_charm)
st.caption(f"⏱️ **Daily Charm Flow:** {charm_display} — estimated dealer delta decay accumulation from time pass today.")
