...
def load_and_compute_gex_engine(ticker, r_rate, target_price, min_d, max_d):
    try:
        ...
        # POLISH FIX 1: Initialize option chain network lookup cache
        chain_cache = {}
        
        try:
            near_chain = stock_obj.option_chain(expirations[0])
            chain_cache[expirations[0]] = near_chain  # Prime cache
        ...
        for exp_str in expirations:
            try:
                ...
                # POLISH FIX 1: Fetch from cache or call API to eliminate extra network trip
        ...
        # POLISH FIX 4: Guard against zero values with an absolute $1 floor limit
...
# POLISH FIX 2: Render bold markdown within a custom small HTML block element
...
with st.container(border=True):
    # POLISH FIX 3: Added orange max pain definition row into the main legend key container
    st.markdown("""
    ### 🔑 Chart Legend Key
