...
if not current_price or (isinstance(current_price, float) and np.isnan(current_price)):
    st.sidebar.error(f"❌ Failed to find market updates for ticker: {ticker_input}")
    st.stop()
...
def load_and_compute_gex_engine(ticker, r_rate, target_price, min_d, max_d):
    try:
        ...
        noise_floor = max(gex_threshold.max() * 0.001, 1.0)
        master_df = master_df[gex_threshold > noise_floor]
        
        if master_df.empty:
            return None, atm_iv_now, max_pain_val, "Filtered Data Empty", 0.0
            
        agg_df = master_df.groupby('strike').agg({
            'GEX': 'sum', 'Vanna': 'sum', 'Charm': 'sum', 'IV_Raw': 'mean'
        }).reset_index()
        
        c_rows = master_df[master_df['Option_Type'] == 'call']
        p_rows = master_df[master_df['Option_Type'] == 'put']
        
        raw_call_split = c_rows.groupby('strike')['GEX'].sum().rename('Call_GEX').reset_index()
        raw_put_split = p_rows.groupby('strike')['GEX'].sum().rename('Put_GEX').reset_index()
        ...
        try:
            ...
            vix_delta = float(vix_hist['Close'].iloc[-1] - vix_hist['Close'].iloc[0]) if len(vix_hist) >= 3 else 0.0
        except Exception:
            vix_delta = 0.0
...
# Promoted error reporting slot to clear sidebar visibility
if data_matrix is None:
    st.sidebar.error("⚠️ No active options data matches your current DTE settings. Change 'Maximum DTE Filter' to 365 to pull global chains.")
    st.stop()
...
with col5:
    st.metric(label="Implied Vol (ATM)", value=f"{atm_iv * 100:.1f}%")
with col6:
...
st.markdown(
    f"<small style='color: gray;'>⏱️ <b>Daily Charm Flow:</b> {charm_display} — estimated dealer delta decay accumulation from time pass today.</small>", 
    unsafe_allow_html=True
)

st.divider()

# --- 8. STRUCTURAL PLAYBOOK GUIDELINES ---
st.subheader("🎯 Structural Playbook Guidelines")

if is_approaching_zero:
    st.warning(f"🚨 **MODEL SIGNAL: TRANSITION ZONE RANGE INTRUSION.** Price is within {pct_from_flip*100:.1f}% of Zero-Gamma node (${zero_gamma_strike:.2f}).")
else:
    st.info(f"ℹ️ **Tipping Point Proximity:** Price is {pct_from_flip*100:.1f}% away from Zero-Gamma Strike (${zero_gamma_strike:.2f}).")

col_pb1, col_pb2 = st.columns(2)
with col_pb1:
    st.markdown(f"### 🟢 ABOVE ESTIMATED ZERO-GAMMA (> ${zero_gamma_strike:.2f})")
    st.markdown("* Historical bias favors lower systemic variance and compressed trading scales.")
with col_pb2:
    st.markdown(f"### 🔴 BELOW ESTIMATED ZERO-GAMMA (< ${zero_gamma_strike:.2f})")
    st.markdown("* Higher statistical tail risks and accelerated intraday expansion metrics.")

st.divider()

# --- 9. PLOTLY CHART COMPONENT & LEGEND KEYS ---
...
with st.container(border=True):
    ...
    * 🟠 **ORANGE DOTTED LINE** = Max Pain: Strike causing maximum option buyer loss at expiry.
...
if chart_mode == "Net GEX Profile":
    fig.add_trace(go.Bar(
        ...
        y=filtered_df['GEX'],
        marker_color=np.where(filtered_df['GEX'] >= 0
